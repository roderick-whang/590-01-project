...
while (st_time + time_step) <= end_time:
    segment = df_data.loc[(df_data['time'] >= st_time) &
                          (df_data['time'] < (st_time + time_step)), :]
    # update activity
...
# # plt.plot(freq_wrist_acc_x[0:100000], np.abs(freq_wrist_acc_x[0:100000])
# /np.max(np.abs(freq_wrist_acc_x)), label="ACC_X")
#
#
#
...
#
# plt.plot(freq_chest_ecg[0:1000000], np.abs(chest_ecg_flat[0:1000000])
# /np.max(np.abs(fft_chest_ecg)), label="PPG signal")
